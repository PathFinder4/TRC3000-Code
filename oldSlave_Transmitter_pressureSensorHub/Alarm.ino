...
void alarm(struct Data sensor_data) 
{
  ...
  delay(5);
  //check all values
  if(venous_pressure_lth > sensor_data.venous_pressure_value || sensor_data.venous_pressure_hth < venous_pressure_value)
  {
    //turn on alarm
    tone(buzzer, 92);
  ...
  if (arterial_pressure_lth > sensor_data.arterial_pressure_value || sensor_data.arterial_pressure_hth < arterial_pressure_value)
  {
    tone(buzzer, 92);
    analogWrite(arterial_LED, 1023);
    alarm_triggered = 1;
  }
  if (inflow_pressure_lth > sensor_data.inflow_pressure_value || sensor_data.inflow_pressure_hth < inflow_pressure_value)
  {
    tone(buzzer, 92);
    analogWrite(inflow_LED, 1023);
    alarm_triggered = 1;
  }
  if (air_detector_lth > sensor_data.air_detector_value || sensor_data.air_detector_hth < air_detector_value)
  {
    tone(buzzer, 92);
    analogWrite(air_detector_LED, 1023);
    alarm_triggered = 1;
  }
  if (blood_leakage_lth > sensor_data.blood_leakage_value || sensor_data.blood_leakage_hth < blood_leakage_value)
  {
    tone(buzzer, 92);
